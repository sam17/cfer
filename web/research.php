	<!-- start header -->
	<?php
	  include 'header.php';
	?>
	<section class="callaction">
	<div class="container">
		<div class="row">
			<div class="col-lg-12">
				<div class="big-cta">
					<div class="cta-text">
						<h2><span>R</span>esearch </h2>
					</div>
				</div>
			</div>
		</div>
	</div>
	</section>
	<section id="content">
	<div class="container" align="justify">
		<div class="row" align="justify">
			<div class="col-lg-12">
				<div class="clearfix">
				</div>
				<div class="row">
					<section id="projects">
					<ul id="thumbs" class="research" >
						<!-- Item Project and Filter Name -->
						<li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="graphic" style="padding-top:30px;">
						<img src="img/groups/quad.jpeg" />
						</li>
						<div align="justify">
						<h4>Autonomous Aerial Navigation</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					    The aim of this project is to create an autonomous quadrotor, capable of balancing itself,

and will be capable of stable flight.
					 </em>
				</p> <br/>
						<b>Professor-in charge : Prof. Somesh Kumar(PI),Prof. J. Mukhopadhyay(Technical Mentor),Prof. D.K. Pratihar(Technical Mentor)</b><br/>
                          Link to page : <a href="aerial.php">LINK</a>
                          </div><br/><hr/>

						<li class="item-thumbs col-lg-3 design" data-id="id-0"  style="padding-top:30px;">
						<img src="img/groups/agv.jpg" ></li>
						<div>
						<h4>Autonomous Ground Vehicle (A.G.V)</h4><br/>
                         <b> Abstract:</b><p>
					<em>
					    Team AGV envisions making an autonomous vehicle that is not only practically feasible 
						but also economically viable.
					 </em>
				</p> <br/>
				<b>Professor-in charge : Prof. D. Chakravarty</b><br/>
                          Link to page : <a href="agv.php">LINK</a>
                          </div><br/><br/><hr/>

					<li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="web" style="padding-top:30px;">
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>

						<div align="justify">
						<h4>Biomedical Robotics for neuro-controlled prosthesis</h4><br/>
                         <b> Abstract:</b><p>
					<em>
					Robotic systems for assistance and rehabilitation focus on providing missing movements 

and sensing, providing safer environments, and providing environments that make 

regaining movement-related function easier and faster.
					 </em>
				</p> <br/>
				<b>Professor-in charge : Prof. M. Manjunatha</b><br/>
                          Link to page : <a href="biomedical.php">LINK</a>
                          </div><br/><br/><hr/>
						
						<li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="graphic"  style="padding-top:30px;">
						<img src="img/groups/swarm.jpeg" />
						</li>
						<div align="justify">
						<h4>Decentralized Terrain Exploration with Robot Swarms</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					 The aim of this project is to design a team of coordinating robots which use swarm 

intelligence to achieve a number of objectives related to navigation and mapping in 

successive stages.
					 </em>
				</p> <br/>
				<b>Professor-in charge : Prof. Somesh Kumar,Prof. P. Dasgupta</b><br/>
                          Link to page : <a href="terrain_exploration.php">LINK</a>
                          </div><br/><br/><hr/>

                          <li class="item-thumbs col-lg-3 photography" data-id="id-4" data-type="web" style="padding-top:30px;">
						<!-- Fancybox - Gallery Enabled - Title - Full Image -->
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>Design and Development of Orthotic Devices for Knee and Hip Joints during Walking of Disabled Person</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					    Rehabilitation robotics plays an important role in taking care of older people to regain 

their physical activity. Orthotic devices are used as artificial instruments to support the 

skeletal system to modify proper functional characteristics of neuromuscular activity.
					 </em>
				</p> <br/>
				<b>Professor-in charge : Prof. D.K. Pratihar,Prof. S. Gupta,Mr. Abhishek Rudra Pal( Ph.D. stuednt)</b><br/>
                          Link to page : <a href="disabled.php">LINK</a>
                          </div><br/><br/><hr/>
						<!-- End Item Project -->
						<!-- Item Project and Filter Name -->
						<li class="item-thumbs col-lg-3 design" data-id="id-1" data-type="icon" style="padding-top:30px;">
						<!-- Fancybox - Gallery Enabled - Title - Full Image -->
						<img src="img/groups/krssg.png" height="200px;">
						</li>
						<div align="justify">
						<h4>Kharagpur Robot Soccer Students' Group (K.R.S.S.G)</h4><br/>
                        <ul> <li><b> Abstract:</b><p>
					<em>
					   KRSSG is a student research group under the Department of Computer Science and 
					   Engineering, primarily working on autonomous soccer playing robots.
					 </em>
				</p> <br/>
					<b>Professor-in charge : Prof. J. Mukhopadhyay</b><br/>

                         <li> Link to page : <a href="krssg.php">LINK</a></li></ul>
                          </div><br/><hr/>
						<!-- End Item Project -->
						<!-- Item Project and Filter Name -->
						<li class="item-thumbs col-lg-3 photography" data-id="id-2" data-type="graphic" style="padding-top:30px;">
						<!-- Fancybox - Gallery Enabled - Title - Full Image -->
						<!-- Thumb Image and Description -->
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>Robot as a Social Assistant</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					    This project is to develop a robotic framework with potential benefits in providing
						assistance to elderly people or having cognitive disorder.
					 </em>
				</p> <br/>
						<b>Professor-in charge : Prof. J. Mukhopadhyay</b><br/>

                          Link to page : <a href="rsa.php">LINK</a>
                          </div><br/><hr/>

                          <li class="item-thumbs col-lg-3 photography" data-id="id-2" data-type="graphic" style="padding-top:30px;">
						<!-- Fancybox - Gallery Enabled - Title - Full Image -->
						<!-- Thumb Image and Description -->
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>Robotic Framework for exploring rugged and potentially hazardous terrain</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					This project is to develop a robotic non-wheeled framework having the capability to go 
					through the utmost rugged terrains as well as to exercise immense stability.
					 </em>
				</p> <br/>
						<b>Professor-in charge : Prof. J. Mukhopadhyay</b><br/>

                          Link to page : <a href="hazardousterrain.php">LINK</a>
                          </div><br/><hr/>
						
						<!-- End Item Project -->
						<!-- Item Project and Filter Name -->
						<li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="web" style="padding-top:30px;">
						<!-- Fancybox - Gallery Enabled - Title - Full Image -->
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>Robotics for Advanced Manufacturing Applications</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					    The objectives of our research are (1) to develop intelligent sensor-based robotic system 
for material handling and assembly applications and (2) to develop sensor integrated 
multi-finger robot grippers for applications in manufacturing.
					 </em>
				</p> <br/>
                       	<b>Professor-in charge : Prof. S. Deb</b><br/>
 
                          Link to page : <a href="manufacturing.php">LINK</a>
                          </div><br/><br/><hr/>

                          <li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="graphic" style="padding-top:30px;">
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>Undergraduate Research In Humanoid Robotics</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					Humanoid robotics with bipedal walking has attracted several researchers and bright 

students worldwide, as these robots challenge the young minds with interesting problems.
					 </em>
				</p> <br/>
				<b>Professor-in charge : Prof. C.S. Kumar</b><br/>
                          Link to page : <a href="humanoid.php">LINK</a>
                          </div><br/><br/><hr/>

                          <li class="item-thumbs col-lg-3 design" data-id="id-0" data-type="graphic" style="padding-top:30px;">
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>Undergraduate Research Support for Underwater Robotics</h4><br/>
                         <b> Abstract :</b><p>
					<em>
					    The objective of this project is to facilitate a continuing learning and development 

autonomous systems in underwater robotics.
					 </em>
				</p> <br/>
				<b>Professor-in charge : Prof. C.S. Kumar</b><br/>
                          Link to page : <a href="underwater.php">LINK</a>
                          </div><br/><br/><hr/>

						<li class="item-thumbs col-lg-3 photography" data-id="id-5" data-type="icon" style="padding-top:30px;">
						<img src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRasqdTGOb9kA4i5nCLFytAlx_W_5_wS2nFKlshnRb-EzrcO5Ve4A" height="150px;">
						</li>
						<div align="justify">
						<h4>User Emotion Classification from Keyboard Keystroke</h4><br/>
                         <b> Abstract:</b><p>
					<em>This project aims to develop a system capable of classifying user emotion (1. Happiness, 

2. Disgust, 3. Anger, 4. Fear, 5. Sadness, 6. Surprise and 7. Neutral.) based on timing 

data and relative position data of consecutive keystrokes (while typing on a keyboard).
					  					 </em>
				</p> <br/>
								<b>Professor-in charge : Prof. P.P. Chakrabarti</b><br/>

                          Link to page : <a href="emotion.php">LINK</a>
                          </div> <br/><br/><hr/>
					</ul>
					</section>
				</div>
			</div>
		</div>
	</div>
	</section>
	<?php
	  include 'footer.php';
	?>
</div>
<a href="#" class="scrollup"><i class="fa fa-angle-up active"></i></a>
<!-- javascript
    ================================================== -->
<!-- Placed at the end of the document so the pages load faster -->
<script src="js/jquery.js"></script>

<script src="js/bootstrap.min.js"></script>
<script src="js/jquery.fancybox.pack.js"></script>
<script src="js/jquery.fancybox-media.js"></script>
<script src="js/google-code-prettify/prettify.js"></script>
<script src="js/portfolio/jquery.quicksand.js"></script>
<script src="js/portfolio/setting.js"></script>
<script src="js/jquery.flexslider.js"></script>
<script src="js/animate.js"></script>
<script src="js/custom.js"></script>
<script src="js/jquery.easing.1.3.js"></script>
</body>
</html>